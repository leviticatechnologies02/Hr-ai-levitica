...
const TerminationModal = ({ isOpen, onClose, onSubmit, employee = null }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Terminate Employee - ${employee.name}`} size="md">
      <form onSubmit={handleSubmit} className="space-y-4 p-2">
        <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl">
          <p className="text-sm text-rose-700">
            Terminating <strong>{employee.name}</strong> ({employee.employeeId})
          </p>
        </div>

        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1.5">
            Reason for Termination <span className="text-rose-500">*</span>
          </label>
          <textarea
            name="reason"
