...
import { inductionAPI } from '../../../shared/utils/api';
...
const InductionOrientation = () => {
  ...
  // Backend ProgramResponse -> local UI shape. The backend's Program
  // record is much leaner than this form (no location/meetingLink/
  // department/programLead/autoAssign/sendNotifications/
  // generateCertificate) — those fields aren't persisted anywhere yet,
  // so they're defaulted here rather than invented.
  const mapProgram = (p) => ({
    id: p.id,
    name: p.name,
    ...
    totalSessions: 0,
    completedSessions: 0,
    totalParticipants: p.enrolled_count || 0,
  ...
  const mapPolicy = (pol) => ({
    ...
    status: pol.status,
    totalEmployees: pol.total_employees,
    completedEmployees: pol.completed_employees,
    completionPct: pol.completion_pct,
    totalModules: pol.total_modules,
  ...
  const mapParticipant = (p) => {
    const attendanceMap = { Present: 'present', Absent: 'absent' };
    return {
      id: p.id,
      employeeId: p.employee_code || String(p.employee_id),
      ...
      programAssigned: p.program_id,
      attendanceStatus: attendanceMap[p.attendance] || 'not_started',
      status: 'Active',
      rating: p.rating,
    };
  };
  ...
  const loadInductionData = async () => {
    try {
      const [statsData, programsData, policiesData, sessionsData, participantsData] = await Promise.all([
        inductionAPI.getStats().catch((err) => { console.error('Error loading induction stats:', err); return null; }),
        inductionAPI.listPrograms().catch((err) => { console.error('Error loading induction programs:', err); return []; }),
        inductionAPI.listPolicies().catch((err) => { console.error('Error loading induction policies:', err); return []; }),
        inductionAPI.listSessions().catch((err) => { console.error('Error loading induction sessions:', err); return []; }),
        inductionAPI.listParticipants().catch((err) => { console.error('Error loading induction participants:', err); return []; }),
      ]);

      setStats(statsData);
      setInductionPrograms(Array.isArray(programsData) ? programsData.map(mapProgram) : []);
      setPolicies(Array.isArray(policiesData) ? policiesData.map(mapPolicy) : []);
      setSessions(Array.isArray(sessionsData) ? sessionsData.map(mapSession) : []);
      setEmployees(Array.isArray(participantsData) ? participantsData.map(mapParticipant) : []);
    } catch (err) {
      console.error('Error loading induction & orientation data:', err);
    }
  };
  ...
  const handleCreateProgram = () => {
    ...
    inductionAPI.createProgram({
      name: programForm.name,
      description: programForm.description || undefined,
      type: programForm.type || 'batch',
      status: 'Upcoming',
      start_date: programForm.startDate,
      end_date: programForm.endDate,
      max_participants: programForm.maxParticipants || undefined,
    })
      .then(() => loadInductionData())
      .catch((err) => {
        console.error('Error creating induction program:', err);
        alert('Failed to save the program to the server: ' + (err.message || 'Unknown error'));
      });
  ...
  const handleCreateSession = () => {
    ...
    // NOTE: meetingLink, venue, and trainer aren't in the backend's
    // SessionCreate schema — there's nowhere for them to persist yet,
    // so they stay local-only for now (same gap as the trainer/venue/
    // material handlers flagged earlier).
    inductionAPI.createSession({
      program_id: sessionAgendaForm.programId,
      title: sessionAgendaForm.sessionTitle,
      description: sessionAgendaForm.agenda || undefined,
      ...
      .then(() => inductionAPI.listSessions())
      .then((sessionsData) => {
      ...
      .catch((err) => {
        console.error('Error creating induction session:', err);
        alert('Failed to save the session to the server: ' + (err.message || 'Unknown error'));
      });
  ...
  const handleEditSession = () => {
    ...
    inductionAPI.updateSession(editingSession.id, {
      title: editingSession.sessionTitle,
      description: editingSession.agenda || undefined,
      ...
      .catch((err) => {
        console.error('Error updating induction session:', err);
        alert('Failed to save session changes to the server: ' + (err.message || 'Unknown error'));
      });
  ...
  const handleDeleteSession = (sessionId) => {
    if (window.confirm('Are you sure you want to delete this session? This action cannot be undone.')) {
      ...
      inductionAPI.deleteSession(sessionId)
        .catch((err) => console.error('Error deleting induction session:', err));
  ...
  const handleUpdateProgramStatus = (programId) => {
    ...
    if (program) {
      ...
      } else if (nextStatus === 'Ongoing' && new Date(program.startDate) > new Date()) {
        updatePayload.start_date = new Date().toISOString().split('T')[0];
      }

      inductionAPI.updateProgram(programId, updatePayload)
        .catch((err) => console.error('Error updating program status:', err));
  ...
  const handleDeleteEmployee = (employeeId) => {
    if (window.confirm('Are you sure you want to delete this employee?')) {
      ...
      inductionAPI.removeParticipant(employeeId)
        .catch((err) => console.error('Error removing induction participant:', err));
